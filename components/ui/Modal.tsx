...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, footer, contentClassName }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 isolate">
      <div className="absolute inset-0 bg-slate-900/55 transition-opacity" onClick={onClose} />
      <div className={`relative w-full bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-visible animate-in fade-in zoom-in-95 duration-150 ${contentClassName ?? 'max-w-lg'}`}>
